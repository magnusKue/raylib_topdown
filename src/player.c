...
void player_update_velocity_by_input(player_t* player) {
    ...
    // player->velocity += inputs * player->acceleration;
    player->velocity = Vector2Add(player->velocity, Vector2Scale(Vector2Normalize(inputs), player->acceleration));
...
void player_update_state(player_t* player) {
    if (Vector2Length(player->velocity) > 5.0) {
